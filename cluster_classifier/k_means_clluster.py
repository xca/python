...
class cluster:
    ...
    def write_cluster_file(self):
        ...
        request1 = 'https://user:YOUR USER KEY FOR BING API=@api.datamarket.azure.com/Bing/Search/News?Query=%27'
    ...
    def k_means(self, k):
        ...
        maxiter = 50  
        #random seeds
        #print "k_means starts"
        centroids = []
        for i in xrange(k):
            #print i
            if centroids == []:
                centroids.append(self.docVector[random.choice(self.doc_words.keys())])

            else: #only when the new centroid is not too close to any of the existing centroids, it will be added to the centroids[]
                c = self.docVector[random.choice(self.doc_words.keys())]
                flag1 = True
                #count = 0
                while(flag1):
                    maxCosine = 0
                    #if count < 10:
                        #print count,len(centroids)
                    for v in centroids:
                        sim = self.cosine(c, v)
                        #print "k_means: while loop",sim
                        if sim > maxCosine:
                            maxCosine = sim
                    #print "k_means: maxCosine: ", maxCosine
                    if maxCosine < 0.5:
                        centroids.append(c)
                        flag1 = False
                        #print flag1,
